...
__m256i mandelbrot_256(__m256 real, __m256 imag, int limit, __mmask8 mask)
{
...
void BatchMandelCalculator::batch8x8(int i_from, int j_from)
{
	int i_limit = i_from + BATCH_SIZE_8;
	...
	if (diff < BATCH_SIZE_8)
		mask >>= BATCH_SIZE_8 - diff;
	...
	//	prepare j
		__m512d j_pd = _mm512_add_pd(_mm512_set1_pd(static_cast<double>(j_from)), inc_pd);

	//	x = x_start + j * dx
		__m512d x_pd = _mm512_add_pd(x_start_pd, _mm512_mul_pd(j_pd, dx_pd));
		__m256 x = _mm512_cvtpd_ps(x_pd);

		__m256i values = mandelbrot_256(x, y, limit, mask);
...
{
	int i_limit = i_from + BATCH_SIZE_16;
	...
	int diff = width - j_from;
	if (diff < BATCH_SIZE_16)
		mask >>= BATCH_SIZE_16 - diff;
	...
	//	x = x_start + j * dx
		...
		__m512i values = mandelbrot_512(x, y, limit, mask);
